...
// Start the auto-next countdown at the same 95% point. Most CMS videos have
// 20–60s of credits/ads at the tail; waiting for real `ended` feels laggy.
const AUTO_NEXT_THRESHOLD = 0.95;
...
export function PlayerEmbed({
	...
	// null = idle; number = seconds remaining until auto-advance fires.
	const [autoNextSecs, setAutoNextSecs] = useState<number | null>(null);
	// Explicit cancel sticks for the rest of this episode — the `video:ended`
	// fallback should also respect it.
	...
	useEffect(() => {
		...
		// Kick the auto-next countdown exactly once per mount when we cross 95%.
			...
			try {
				...
				if (progressKey) {
					...
					progressTimer = setInterval(() => {
						...
						// Kick the auto-next countdown once we cross 95%. Skip when the
						// user paused — they're clearly not done watching this episode.
						if (
							!countdownStarted &&
							...
							Number.isFinite(d) &&
							d > 0 &&
							t / d >= AUTO_NEXT_THRESHOLD &&
							!a.video.paused
						) {
							countdownStarted = true;
							setAutoNextSecs(AUTO_NEXT_COUNTDOWN);
						}
	...
	// Reset auto-next state whenever the episode changes. A fresh episode
	// gets a fresh countdown chance, regardless of whether the previous one
	// was cancelled.
	useEffect(() => {
		autoNextCancelledRef.current = false;
		setAutoNextSecs(null);
	}, [progressKey]);

	// Countdown tick. Decrements once per second; at 0, fires the navigation.
	// Re-enters on each state change so a cancel (secs → null) stops it cleanly.
	useEffect(() => {
		if (autoNextSecs === null) return;
		if (autoNextSecs <= 0) {
			const href = nextHrefRef.current;
			setAutoNextSecs(null);
			if (href) onNavigate(href);
		...
		const id = setTimeout(() => {
			setAutoNextSecs((n) => (n === null ? null : n - 1));
		}, 1000);
		return () => clearTimeout(id);
	}, [autoNextSecs, onNavigate]);
	...
	return (
		<div>
			<div className="relative overflow-hidden rounded-lg border border-border bg-black">
				...
				{helpOpen ? (
					<KeyboardShortcutsOverlay onClose={() => setHelpOpen(false)} />
				) : null}
				{autoNextSecs !== null && !error ? (
					<div className="pointer-events-none absolute right-3 top-3 z-30 flex items-center gap-3 rounded-full bg-black/75 px-3.5 py-1.5 text-xs text-white shadow-lg backdrop-blur-sm">
						<span className="tabular-nums">{autoNextSecs} 秒后播放下一集</span>
						<button
							type="button"
							onClick={() => {
							...
							className="pointer-events-auto rounded-full bg-white/15 px-2.5 py-0.5 text-[11px] transition-colors hover:bg-white/25"
						>
							取消
						</button>
					</div>
	...
	return (
		...
		>
			<div className="w-full max-w-sm rounded-lg border border-border bg-surface/90 p-5 shadow-xl backdrop-blur">
				...
				<p className="mt-4 text-[10px] text-dim-foreground">
					按 Esc 关闭 · 按 ? 再次打开
				</p>
			</div>
		</div>
